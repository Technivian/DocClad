...
|theme/templates/contracts/organization_activity.html|NetworkPage|QueuePage|Medium|Audit log list with filters + pagination — QueuePage pattern|**Batch 5 Step 5 discovery (2026-05-18)** — 28 raw Tailwind hits, 2× `onchange` handlers (preserve), no destructive actions. Slice B candidate.|Medium|Medium|Full QueuePage migration safe after Slice A; preserve onchange handlers|
|theme/templates/contracts/organization_identity_settings.html|QueuePage|WorkspacePage|Medium|Settings form workspace with multiple panels (SAML/SCIM/API tokens/webhooks)|**Batch 5 Step 5 discovery (2026-05-18)** — already uses settings design system; 2× `btn-primary` + 2× `btn-secondary` undefined → fix to `btn-primary-grad`/`btn-ghost`; 2× `onsubmit confirm` guards preserved. Slice A candidate.|Medium|Low|Fix 4 undefined button classes; preserve both onsubmit destructive confirms|
|theme/templates/contracts/organization_security_settings.html|QueuePage|WorkspacePage|Medium|MFA policy + session revocation settings form workspace|**Batch 5 Step 5 discovery (2026-05-18)** — already uses settings design system; undefined: `btn-primary` → `btn-primary-grad`, `btn-secondary` → `btn-ghost`, `ds-badge` → `badge-sm`, `checkbox-primary` → remove class; 1× `onsubmit confirm` preserved. Slice A candidate.|Medium|Low|Fix 4 undefined classes; ds-badge → badge-sm; preserve onsubmit confirm|
|theme/templates/contracts/organization_session_audit.html|ExceptionPage|QueuePage|Medium|Active sessions list with revoke actions — QueuePage pattern|**Batch 5 Step 5 discovery (2026-05-18)** — already uses settings design system; 1× `btn-secondary` undefined → `btn-ghost`; 1× raw Tailwind border utility; 1× `onsubmit confirm` preserved. Slice A candidate.|Low|Low|Fix btn-secondary → btn-ghost; preserve onsubmit confirm|
...
|theme/templates/settings_hub.html|CommandPage|WorkspacePage|High|Settings hub/nav landing — links to all settings areas; already uses settings design system|**Batch 5 Step 5 discovery (2026-05-18)** — zero raw Tailwind, zero forms, uses `page-container`/`settings-grid`/`settings-card` (all defined in base.html). Only change: `page-container` → `page-wrap`. Slice A candidate.|Low|Low|Minimal — swap page-container wrapper to page-wrap; archetype reclassified to WorkspacePage|
