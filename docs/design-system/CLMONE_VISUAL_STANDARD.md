# CLM One Visual & Product Standard

Converted from `CLM One UI Redesign Blueprint v1.docx` so agents and contributors can diff, cite, update, and follow it without parsing a Word file. This is the standard — the `.docx` (archived alongside this file) is not a source of truth.
...
CLM One should not feel like a document storage tool with forms attached.
...
This blueprint should guide product, design, and engineering decisions so the redesign does not drift into random screens or isolated UI improvements. That matters especially because CLM One's own governance model treats its Charter as the source of truth for product/design/engineering alignment.

## 2. Core Product Model

CLM One is workflow-first. The main product spine is:
...
```
User selects DPA
→ CLM One starts DPA Privacy Review Workflow
...
**Design feeling:** this screen should feel like CLM One knows what needs attention before the user does.
...
Header: *"What are you creating?"* / *"Select a contract type. CLM One will apply the right approved template, playbook, clause logic, and approval route."*
...
> This Data Processing Agreement is entered into between CLM One B.V. and **[Counterparty Name]** as of **[Effective Date]**. The Processor shall process personal data only for the purposes described in **[Processing Purpose]**. Data transfers outside the EEA: **[Data Transfer Position]**.
...
**Purpose:** the legal intelligence screen — where CLM One proves it is more than drafting. It should answer: What risks did we find? What does the playbook say? What clause is problematic? What fallback should we use? Who needs to approve? Can we generate a memo?
...
2. User selects DPA
3. CLM One starts DPA Privacy Review Workflow
4. User fills required fields
5. User answers privacy smart questions
6. Live DPA draft updates
7. CLM One detects: personal data involved, subprocessors involved, possible EEA transfer
...
**Bad:** "No items found."
**Better:** "No priority legal work is blocking the business. CLM One is still monitoring approvals, renewals, DPA reviews, and risk exceptions."
