...
| Verified missing baseline | **Missing:** canonical-read parity evidence; reconciled `DPAReviewPack.approval_status`; required `ApprovalRoute` mapping; ABSTAIN and explicit REVOKE UI/actions; cutover and rollback rehearsal; authorization for legacy read retirement. The 40-file ownership matrix is completed evidence, not a remaining gap. |
...
| DPA reconciliation decision | **Deferred pending PDR-0005 acceptance.** The proposed combined ADR/PDR recommends DPA as a specialist workflow gate, but no decision is binding until repository governance accepts it. Do not introduce a local DPA-to-generic relationship or infer one from current rows. Privacy Review retains its specialist, human-controlled DPA state; generic approval remains separately authoritative. |
| Next planning slice | Review the proposed PDR-0005 only. No reconciliation implementation is authorized unless the proposal is accepted and a separate additive implementation scope is approved. |
...
| Proposed phases | **0 — entry gates and ownership: Completed for characterization only. 1 — characterization: Completed; exception expired on PR #93 merge. 2 — DPA reconciliation planning and evidence inventory: Completed; broader approval-domain decision package: Proposed (PDR-0005); residual reconciliation implementation: Blocked pending acceptance and separate authorization. 3 — reversible read cutover: Blocked pending a separate authorization. 4 — retirement decision: Deferred and separately governed.** |
...
| 2026-07-24 | **PAR-APR-002 combined decision proposed:** PDR-0005 recommends DPA as a specialist workflow gate, retaining separate Privacy Review and generic Approval ownership. It remains Proposed and authorizes no implementation, mapping, migration, authority change, or legacy retirement. |
