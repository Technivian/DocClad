...
| DPA inventory finding | **Evidence-only inventory prepared:** no valid persisted DPA-to-generic-approval linkage exists; shared contract, organization, reviewer, timing, or status label is insufficient. All DPA statuses remain unmappable until a separately governed linkage rule exists. Fixture-only counts and dependency evidence are in [`DPA_INVENTORY_EVIDENCE.md`](../audits/evidence/2026-07-22-par-apr-002/DPA_INVENTORY_EVIDENCE.md). |
| Next planning slice | No reconciliation slice is currently authorized. The smallest scope to submit for separate authorization is evidence-only: inventory DPA-to-generic linkage candidates and status-mapping/mismatch counts per organization, with cross-tenant and permission characterization. No code, migration, read-authority, dual-write, permission, repair, flag, or UI/API behaviour change. |
...
| Proposed phases | **0 — entry gates and ownership: Completed for characterization only. 1 — characterization: Completed; exception expired on PR #93 merge. 2 — DPA reconciliation planning: Completed; evidence-only residual reconciliation: Planned and separately authorized. 3 — reversible read cutover: Blocked pending a separate authorization. 4 — retirement decision: Deferred and separately governed.** |
