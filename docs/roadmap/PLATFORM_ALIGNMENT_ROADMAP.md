...
| Verified missing baseline | **Missing:** canonical-read parity evidence; governed `ApprovalRoute` identity and route-to-requirement mapping; ABSTAIN and explicit REVOKE UI/actions; cutover and rollback rehearsal; authorization for legacy read retirement. DPA planning and ApprovalRoute characterization are completed evidence, not runtime reconciliation. The 40-file ownership matrix is completed evidence, not a remaining gap. |
...
| Next planning slice | No implementation is authorized by this decision. The smallest safe slice is a separate planning/authorization package for immutable route identity, ownership, route-to-requirement creation semantics, duplicate/ambiguous/stale invalidation, and tenant/version binding. It must not introduce a model, migration, mapping, dual-write, or read cutover. |
...
| Proposed phases | **0 — entry gates and ownership: Completed for characterization only. 1 — characterization: Completed; exception expired on PR #93 merge. 2 — DPA and ApprovalRoute reconciliation planning/evidence: Completed; broader approval-domain decision package: Accepted (PDR-0005); residual reconciliation implementation: Blocked pending separate authorization. 3 — reversible read cutover: Blocked pending a separate authorization. 4 — retirement decision: Deferred and separately governed.** |
